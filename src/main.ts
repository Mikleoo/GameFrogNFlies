...
let obj : circuit = new circuit(10, 10, 0); // On load notre jeu avec des dimension de 10x10 et un score de 0
...
// On définit un nouveau type "data" qui permet d'avoir à l'intérieur :
// - tags : Objet qui contient les coordonnées x et y avec son "type" (attribut HTML)
// - flies : Tableau d'objet qui contient des objets avec les valeurs  : name (mouche avec un numéro) et les coordonnées de chaque mouches
// - frog : Objet qui contient son nom (Grenouille par default mais inutilisé), ses coordonnées et sa class (qui aurait pu servir mais inutilisé dans le code)
type data = {
  tags : {
    x: number,
    y: number,
...
// On définit un objet "matrice" avec comme type "data"
...
function newGame() {
  ...
  obj.generate(5);
  obj.checkGenerate();
  document.getElementById("nombredetours")!.innerHTML = "Tour : 0";
}
